...
namespace KMLToSHP
{
    public sealed partial class MainForm : Form
    {
        ...
        private void gDB2SHPToolStripMenuItem_Click(object sender, EventArgs e)
        {
            string sGDBFiles = @"D:\77211356\GVG\解译后\gdb5";
            List<string> list = FileManage.getAllFileNameFromFolder(sGDBFiles, "lyr");
            string sOutPutFolder = @"D:\77211356\GVG\解译后\shp";
            foreach (var item in list)
            {
                item.Replace("lyr", "gdb");
                string sFileName = Path.GetFileNameWithoutExtension(item);
                string sGDBName = sGDBFiles+"\\"+sFileName + ".gdb";
                SpatialDBManage.ConvertFeatureClassToShapefile(sGDBName, sOutPutFolder, "Polylines", sFileName);
            }
